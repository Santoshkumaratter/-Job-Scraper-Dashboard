"""Reed Job Scraper - MULTI-APPROACH: Tries multiple methods to fetch maximum jobs"""
...
class ReedScraper(BaseScraper):
    ...
    def scrape_jobs(self) -> List[Dict]:
        ...
        for keyword in self.keywords:
            ...
            url_formats = [
                ...
                f"{self.base_url}/jobs?keywords={urllib.parse.quote(keyword)}",
            ...
            if not soup:
                logger.warning(f"Reed: Could not fetch HTML for keyword '{keyword}'")
                continue
            ...
            if not job_cards:
                logger.warning(f"Reed: No job cards found for '{keyword}' after trying all approaches")
                continue
            
            logger.info(f"Reed: Found {len(job_cards)} job cards using multi-approach for '{keyword}'")
                ...
                try:
                    ...
                    if hasattr(card, 'find'):
                        title_elem = card.find('h2') or card.find('h3') or card.find('h1') or \
                                   ...
                                   card.find('a', class_='job-result-heading__link')
                    ...
                    if card.name == 'a':
                        ...
                        link_elem = card.find('a', class_='job-result-heading__link') or card.find('a')
                    ...
                    posted_date = self.parse_date(card.find('time').get('datetime') if hasattr(card, 'find') and card.find('time') else '')
                    location = self.clean_text(card.find('li', class_='job-metadata__item').get_text() if hasattr(card, 'find') and card.find('li', class_='job-metadata__item') else '') or ''
                    ...
                    try:
                        ...
                        logger.debug(f"Reed: Error fetching job detail for {job_link}: {e}")
    ...
    def _fetch_job_detail(self, job_link: str) -> Dict[str, Optional[str]]:
        ...
        try:
            ...
            logger.debug(f"Reed: Error fetching job detail: {e}")
